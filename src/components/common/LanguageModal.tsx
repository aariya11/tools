...
export const LanguageModal: React.FC<LanguageModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm animate-in fade-in duration-150">
      <div 
        className="w-full max-w-2xl rounded-3xl bg-[#161513] border border-[#2A2824] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-150 flex flex-col max-h-[85vh]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="p-6 border-b border-[#2A2824] flex items-center justify-between shrink-0">
        ...
        <div className="p-4 border-b border-[#2A2824] bg-[#11110F]/60 space-y-3 shrink-0">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-[#2A2824] bg-[#161513] text-sm text-[#F5F1E8] placeholder:text-[#7A756D] focus:border-[#B79B70] focus:ring-1 focus:ring-[#B79B70] outline-none"
              autoFocus
            />
          </div>
            ...
            {regions.map((reg) => (
              <button
                ...
                className={`px-3 py-1 rounded-lg font-medium shrink-0 transition-colors cursor-pointer ${
                  selectedRegion === reg
                    ? 'bg-[#B79B70] text-[#11110F] font-bold'
                    : 'bg-[#1B1A17] text-[#B8B2A7] hover:text-[#F5F1E8] hover:bg-[#1F1E1B]'
                }`}
              >
                {reg}
              </button>
        ...
        <div className="p-6 overflow-y-auto flex-1">
                ...
                <button
                  ...
                  className={`p-3.5 rounded-2xl border text-left transition-all flex items-center justify-between gap-2 cursor-pointer ${
                    isSelected
                      ...
                      : 'border-[#2A2824] bg-[#161513] hover:border-[#3D3A34] hover:bg-[#1B1A17]/60'
                  }`}
                >
                  <div className="flex items-center gap-3 truncate">
                    ...
                    <div className="truncate text-left">
                      ...
                      <div className="text-[10px] text-[#B8B2A7] truncate">
                        {lang.name}
                      </div>
                    </div>
                  </div>
                  {isSelected ? (
                    <Check className="w-4 h-4 text-[#B79B70] shrink-0" />
                  ) : lang.isRtl ? (
                    <span className="text-[9px] uppercase font-mono font-bold px-1.5 py-0.5 rounded bg-[#1B1A17] border border-[#2A2824] text-[#B8B2A7] shrink-0">
                      RTL
                    </span>
                  ) : null}
                </button>
          ...
          {filteredLanguages.length === 0 && (
            <div className="py-12 text-center text-[#7A756D]">
              <Globe className="w-8 h-8 mx-auto mb-2 opacity-40 text-[#B79B70]" />
              <p className="text-sm font-semibold text-[#B8B2A7]">No languages found matching "{searchQuery}"</p>
              <button
                onClick={() => { setSearchQuery(''); setSelectedRegion('All'); }}
                className="mt-3 text-xs text-[#B79B70] hover:underline font-bold"
              >
                Reset filters
              </button>
            </div>
        ...
        {/* Footer */}
        <div className="p-4 border-t border-[#2A2824] bg-[#11110F] flex items-center justify-between shrink-0">
          <div className="text-xs text-[#B8B2A7]">
            Active: <span className="font-bold text-[#F5F1E8]">{currentLanguage.nativeName}</span>
          </div>
          <button
            onClick={onClose}
