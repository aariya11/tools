...
export function SpotlightNavbar({
  ...
  // Find initial active index based on route if possible
  ...
  const [activeIndex, setActiveIndex] = useState(
    currentPathIndex !== -1 ? currentPathIndex : defaultActiveIndex
  );
  const [hoverX, setHoverX] = useState<number | null>(null);

  // Sync active index when location changes
  useEffect(() => {
          ...
          {items.map((item, idx) => (
            <li key={idx} className="relative h-full flex items-center justify-center">
              <Link
                ...
                className={cn(
                  'px-3.5 py-1.5 text-xs sm:text-sm font-medium transition-colors duration-200 rounded-full select-none',
